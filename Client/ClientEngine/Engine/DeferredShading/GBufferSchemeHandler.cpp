  ...
  Pass* pass, unsigned short lodIndex, const Renderable* rend)
{
  ...
  std::string sMatName = rend->getMaterial()->getName();
  Ogre::StringUtil::toLowerCase(sMatName);

  if (sMatName.find("editor/") != std::string::npos)
  {
  ...
  // UGLY hack
  Ogre::Entity* pEnt = dynamic_cast<Ogre::Entity*>(const_cast<Ogre::Renderable*>(rend));
  Ogre::SubEntity* pSubEnt = NULL;

  if (!pEnt && (pSubEnt = dynamic_cast<Ogre::SubEntity*>(const_cast<Ogre::Renderable*>(rend))) != NULL)
  {
    pEnt = pSubEnt->getParent();
  }
  // !
  ...
  if (/*pass->hasVertexProgram() || */pEnt && pEnt->hasSkeleton())
  {
    //bool bIsSkinned = pass->getVertexProgram()->isSkeletalAnimationIncluded();

    //if (!bIsSkinned && pEnt->hasSkeleton())
      bool bIsSkinned = true;

    props.isSkinned = bIsSkinned;
  }
  else 
  {
  ...
  ushort passNum = pass->getNumTextureUnitStates();

  for (unsigned short i = 0; i < passNum; i++) 
  {
