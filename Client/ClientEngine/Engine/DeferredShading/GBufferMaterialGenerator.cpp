...
Ogre::GpuProgramPtr GBufferMaterialGeneratorImpl::generateVertexShader(MaterialGenerator::Perm permutation)
{
  ...
  if (permutation & GBufferMaterialGenerator::GBP_SKINNED)
  {
    ss << "	uniform float3x4 cWorldMatrix3x4Array[40]," << std::endl;
    ss << " uniform float4x4 cViewProjectionMatrix," << std::endl;
    ss << " uniform float4x4 cInvWorldMatrix," << std::endl;
    ss << " uniform float4x4 cViewMatrix" << std::endl;
  }
  else
  {
  ...
  if (permutation & GBufferMaterialGenerator::GBP_SKINNED)
  {
    ...
    ss << " for (i = 0; i < 3; ++i) {"                                                                            << std::endl;
    ss << "   blendPos += float4(mul(cWorldMatrix3x4Array[blendIdx[i]], iPosition).xyz, 1.0) * blendWgt[i];"      << std::endl;
    ss << " }"                                                                                                    << std::endl;
            
    ss << " oPosition = mul(cViewProjectionMatrix, blendPos);"                                                    << std::endl;
    
    ss << " oNormal = float3(0, 0, 0);"                                                                           << std::endl;
    ss << " for (i = 0; i < 3; ++i) {"                                                                            << std::endl;
    ss << "   oNormal += mul((float3x3)cWorldMatrix3x4Array[blendIdx[i]], iNormal) * blendWgt[i];"                << std::endl;
    ss << " }"                                                                                                    << std::endl;
    ss << " oNormal = mul((float3x3)cInvWorldMatrix, oNormal);"                                                   << std::endl;
    ss << " oNormal = normalize(oNormal);"                                                                        << std::endl;

	  if (permutation & GBufferMaterialGenerator::GBP_NORMAL_MAP)
	  {
      ss << " oTangent = float3(0, 0, 0);"                                                                        << std::endl;
      ss << " for (i = 0; i < 3; ++i) {"                                                                          << std::endl;
      ss << "   oTangent += mul((float3x3)cWorldMatrix3x4Array[blendIdx[i]], iTangent) * blendWgt[i];"            << std::endl;
      ss << " }"                                                                                                  << std::endl;
		  ss << "	oTangent = mul((float3x3)cInvWorldMatrix, oTangent);"                                               << std::endl;
		  ss << "	oTangent = normalize(oTangent);"                                                                    << std::endl;
		  ss << "	oBiNormal = cross(oNormal, oTangent);"                                                              << std::endl;
	  }

#ifdef WRITE_LINEAR_DEPTH
    //ss << "	oViewPos = mul(cWorldView, iPosition).xyz;"                                                         << std::endl;
    ss << "	oViewPos = mul(cViewMatrix, blendPos).xyz;"                                                           << std::endl;
#else
  ...
  else
  {
	...
	ptrProgram->setParameter("profiles","vs_2_0");
  ...
  if (permutation & GBufferMaterialGenerator::GBP_SKINNED)
  {
    params->setNamedAutoConstant("cWorldMatrix3x4Array", Ogre::GpuProgramParameters::ACT_WORLD_MATRIX_ARRAY_3x4);
    params->setNamedAutoConstant("cViewProjectionMatrix", Ogre::GpuProgramParameters::ACT_VIEWPROJ_MATRIX);
    params->setNamedAutoConstant("cInvWorldMatrix", Ogre::GpuProgramParameters::ACT_INVERSE_WORLD_MATRIX);
    params->setNamedAutoConstant("cViewMatrix", Ogre::GpuProgramParameters::ACT_VIEW_MATRIX);
