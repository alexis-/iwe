...
CRenderEngine::setupResources(void)
{
  ...
  CResourceManager::getInstance()->eventUpdateFinished +=
    FastDelegate0<>(this, &CRenderEngine::onResourceUpdated);

	return true;
}

void
CRenderEngine::onResourceUpdated()
{
  static bool bInit = false;

  if (!bInit && m_pViewports.size() > 0)
  {
    CViewport* pViewport = dynamic_cast<CViewport*>(m_pViewports[0]);

    if (pViewport)
    {
      RenderSettings::getInstance()->initialize(pViewport->getOgreViewport(), m_pSceneMgr, pViewport->getCamera()->getOgreCamera());
      bInit = true;
    }
  }
}
